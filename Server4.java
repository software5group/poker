        ...
        try {
            ...
            while (true) {
                ...
                synchronized (this) {
                    ...
                    player1.SortCards();//並び替えてから
                    for(int i=0;i<=4;i++){
                        out.println(player1.hand[i].Toint());//手札を送る
                    }
                    ...
                    if(n != 0){//交換する枚数が0枚だったら終わり
                        ...
                        for(int i=1; i<=n; i++){
                            ...
                            player1.hand[change-1] = deck.Draw();//新しく引いたカードに置き換える
                        }
                    ...
                    for(int i=0;i<=4;i++){
                        out.println(player1.hand[i].Toint());//手札を送りなおす
                    }

                    if (Thread.currentThread().getName().equals("Thread-0")) {
                        player1.rank = Integer.parseInt(in.readLine());;
                        System.out.println("Player1 Rank: " + player1.rank);//ランクを出してスレッド0だったらplayer1に入れる
                    }else{
                        player2.rank = Integer.parseInt(in.readLine());;
...
class Hand{
    ...
    public String Winlose(Hand player1,Hand player2){
        if(player1.rank<player2.rank){
            ...
            for(int i=0;i<=4;i++){
                if(player1.Kicker[i]>player2.Kicker[i]){
                    player1.chip = player1.chip + player1.betchip + player2.betchip;
                    return "あなたの勝利です";
                }else if(player1.Kicker[i]<player2.Kicker[i]){
                    player2.chip = player2.chip + player1.betchip + player2.betchip;
                    return "相手の勝利です";
                }
            }
